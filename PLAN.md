...
### 阶段 6｜设计阶段输入/输出与文件版本管理
**目标：** 固化三阶段输入/输出清单，建立文件归档、版本展示与下载闭环。

**计划：**
- 形成方案/初设/施工图三阶段输入、计算成果、跨专业输出与出图后闭环清单。
- 文件上传必须携带系统、阶段与前置信息，服务端按分类存储。
- 前端默认展示最新版本，支持展开历史版本与跨项目同类型对比浏览。
- 页面仅展示文件名称与附件名称，不做内容对比；下载后自行对比。
- 支持单文件、分类/阶段、全项目的一键下载。
...
### 阶段 7｜设计说明模板与 DWG 图框填充闭环
**目标：** 建立设计说明模板与关键数据表单，自动生成 DOCX/PDF，并批量写入 DWG 图框生成新版本。

**计划：**
- 按“项目类型 + 设计阶段”管理设计说明模板，模板与图框字段清单由用户提供。
- 建立关键数据表单，字段来源于设计输入与计算结果，支持保存与版本化。
- 一键生成设计说明 DOCX/PDF，作为阶段成果版本入库。
- 批量填充已上传 DWG 图框属性，生成“自动填充版”DWG 新版本入库，原件保留。
- 依赖 AutoCAD 命令行脚本执行，明确环境依赖与失败回滚规则。

**交付物：**
- 设计说明模板字段映射表（含图框字段）
- 设计说明生成与归档规则
- DWG 图框批量填充操作规范与失败回滚说明
...
- **版本一致性风险：** 前端展示版本必须与后端真实文件一致，下载链路可用且可追溯。
- **环境依赖风险：** DWG 图框填充依赖 AutoCAD 命令行环境，需显式校验与降级策略。
...
- 阶段切换与历史查看规则清晰可追溯。
- 三阶段输入/输出清单可追溯且与文件分类一致。
- 文件可按阶段与系统浏览、历史版本可展开、可下载。
